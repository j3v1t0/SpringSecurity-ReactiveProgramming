...
@Component
public class JwtProviderImpl implements JwtProvider<AccountClass> {
    @Value("${app.jwt.secret}")
    private String SECRET_KEY;
    @Value("${app.jwt.expiration-in-minute}")
    private String JWT_EXPIRATION;
    @Override
    public String generateToken(AccountClass userDetails) {
        return this.generateToken(userDetails, new HashMap<>());
    }

    @Override
    public String generateToken(AccountClass userDetails, Map<String, Object> extraClaims) {
        return Jwts.builder()
                .setClaims(extraClaims)
                .setSubject(userDetails.getUsername())
                .claim("id", userDetails.getAccountUuid())
    ...
    @Override
    public Boolean isTokenValid(String token, AccountClass userDetails) {
